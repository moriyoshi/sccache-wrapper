...
> Originally consolidated under `winterbaume/.agents/docs/LTM/`. Paths and integration-specific references have been adapted to this standalone repo. The Winterbaume-side scaffolding examples ( `.agents/bin/cargo.sh`, `mise.toml`, `check-build-cache.sh` ) are preserved as concrete deployment examples; the durable lessons are wrapper-level and apply to any integration.

## Summary

`sccache-wrapper` is a Rust compilation cache layer around `sccache`. It exists because ordinary `sccache` does not normalise Rust compiler command lines enough to share cache entries across git worktrees, and because Cargo emits local target paths, output suffixes, and incremental flags that must be filtered or rewritten for safe reuse.
...
- Env var `WB_SCCACHE_WRAPPER_SCOREBOARD` points at a directory. Each cache-miss compilation drops a `<crate>-<short_key>.json` file there with a sibling `.lock` for fslock-based serialised RMW.
...
- Each entry records:
  ...
  - a `contenders` array, where each contender carries `pid`, `role` ( `building` or `waiting` ), `joined_at`, `last_seen`, the `session` id ( extracted from `CARGO_TARGET_DIR`'s `target-<session>` suffix when callers use that convention ), and a `process_tree` chain ( caller -> cargo -> sccache-wrapper )
...
`sccache-wrapper --show-scoreboard` renders the directory in a human-readable form.

End-to-end smoke test ( in the Winterbaume integration ) against a fresh `WB_RUSTC_CACHE_DIR` confirmed entries appear during a workspace build, transition to `ready` with realistic durations ( 1 - 7 s per crate ), contenders empty out cleanly on Drop, and the process tree is captured as `sccache-wrapper -> cargo -> zsh -> claude -> zsh -> tmux`.
...
The same unanchored-replace pattern still exists for `.d` file content rewriting ( `content.replace(EXTFN_PLACEHOLDER, &parsed.extra_filename)` and the corresponding store-side inverse ). In practice extra-filename hashes are unique enough that collisions are unlikely, but it is the same class of bug. Tracked in TODO.md as `d-file-rewrite`.

### Cross-`CARGO_TARGET_DIR` Cache Misses

When the same workspace is built under different `CARGO_TARGET_DIR` values, the wrapper used to rebuild every external crate from scratch, defeating its whole purpose. Any caller that mints per-session `CARGO_TARGET_DIR` paths ( e.g. an agent harness using `target-<session>/` subdirs ) hits this case.
...
1. **Proc-macro deps.** `parse_rustc_args` rejects `crate-type=proc-macro` outright, so the wrapper never caches them and never writes a sidecar. Every consumer of a proc-macro dep ( basically every Rust workspace, given derive macros are ubiquitous ) hit the fallback path.
...
Existing duplicate cache entries from before the fix stay until GC removes them. `sccache-wrapper --gc` groups by `(program_kind, crate_name, metadata)` and keeps the newest per group, so it converges automatically as builds touch each crate again. Manually wiping `$WB_RUSTC_CACHE_DIR` is fine if an immediate reset is desired.

### Bypassing the Wrapper Defeats Cross-Session Cache Sharing

The wrapper only participates when Cargo is invoked with `RUSTC_WRAPPER` pointing at the wrapper binary and with `WB_WORKSPACE_ROOT` / `WB_RUSTC_CACHE_DIR` exported. Hand-setting `CARGO_TARGET_DIR` and running bare `cargo` ( with no `RUSTC_WRAPPER` ) creates a target directory that looks isolated but bypasses the wrapper entirely. The result is a full fresh compile with no cache lookup, no hardlinks from the shared rustc cache, and no `.cachekey` sidecars next to the produced `.rmeta` files.

The durable diagnostic is the sidecar count: zero `.cachekey` files in `<target>/debug/deps/` means `sccache-wrapper` did not run for that target.

A clean-slate verification in the Winterbaume integration built the wrapper twice through `cargo.sh` with two different session ids: first run populated 24 manifests in the rustc cache, second run completed 3.75x faster and representative `.rmeta` files had link count 3 ( cache + first session + second session ). Repeating the same build through bare `cargo` with wrapper env stripped produced zero sidecars and no speed-up, confirming the populated cache was unused.

Lesson: a wrapper script that exports `RUSTC_WRAPPER`, `WB_WORKSPACE_ROOT`, and `WB_RUSTC_CACHE_DIR` is essential to any multi-session integration. The wrapper itself cannot detect "you forgot to set me as RUSTC_WRAPPER" — the user-facing integration script must enforce it. The Winterbaume `cargo.sh` / `check-build-cache.sh` pair is a reference implementation.
...
- `src/main.rs`: cache key computation, incremental stripping, restore/store logic, singleflight locks, diagnostics, `--dump-cache`, GC, and scoreboard registration / heartbeat / `--show-scoreboard`
- `src/scoreboard.rs`: scoreboard on-disk schema and read-modify-write helpers
- `Cargo.toml`: wrapper dependencies, including `sha2`, `fslock`, `serde`/`serde_json`, and `sysinfo`
- `README.md`: operator documentation for environment variables, cache layout, diagnostics, and the scoreboard

The original Winterbaume integration also ships:

- `.agents/bin/cargo.sh` / `cargo.ps1`: per-session `CARGO_TARGET_DIR` minting + wrapper env export
- `.agents/bin/check-build-cache.sh`: doctor script for verifying that a target dir was built through the wrapper
- `mise.toml`: project-local wrapper environment defaults

These are reference deployment artefacts; this crate does not ship them.

## Test Coverage

- Cross-worktree cache hits validated for workspace and external crates ( e.g. `bytes` ) in the Winterbaume integration.
- Host and target duplicate builds validated by confirming crates such as `parking_lot` produced distinct cache entries instead of hardlinked aliases.
...
- `--dump-cache` checked against entries written with dependency metadata and showed resolved deps such as `serde`, `serde_json`, `tracing`.
...
- Cross-session cache-sharing harness check: warmed cache with session id A, restored representative deps as hardlinks with session id B, and confirmed a bare-cargo bypass build produced zero `.cachekey` sidecars.
...
- Do not run bare `cargo` with a hand-set `CARGO_TARGET_DIR`. That still bypasses the wrapper and prevents cross-session cache sharing. Use the integration's wrapper script ( e.g. Winterbaume's `.agents/bin/cargo.sh` ) and check `.cachekey` sidecar counts if a build unexpectedly recompiles warm dependencies.
- Do not debug long build hangs only at the current crate. First check whether the shared sccache server is responsive.
- Do not use `String::replace` to substitute the `extra-filename` suffix in stored output filenames or `.d` content. It is unanchored and will mangle names whenever the suffix appears mid-stem ( e.g. `stored_ef = "-d"` inside `libred-d.rlib` ). Use `rewrite_extra_filename` ( anchored to the end of the stem ) for filenames; the `.d` content path still uses `String::replace` and is tracked in TODO.md as `d-file-rewrite`.
...
- Mixing wrapper feature sets ( e.g. flipping between bundled and unbundled native deps in the same session ) can resurrect stale artefacts and produce `E0463: can't find crate for X` style errors. Workaround: wipe `$WB_RUSTC_CACHE_DIR` and the session target directory when switching feature sets locally. CI is unaffected — each run starts from a clean cache.
