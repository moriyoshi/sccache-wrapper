...
This skill is an orchestration layer around `good-sleep`. It should verify coverage first, run the `good-sleep` workflow only for missing material, and then normalise the consolidation history in `JOURNAL.md` into one canonical record.
...
5. Repo-authored documentation still uses British English, half-width parentheses, and half-width colons.
...
- If the audit shows that LTM is already complete, skip the `good-sleep` rewrite step and only normalise the consolidation records.
