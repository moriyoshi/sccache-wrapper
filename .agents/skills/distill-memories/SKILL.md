...
a. `.agents/docs/OVERVIEW.md` ... focused on high-level system understanding (scope, boundaries, ecosystem).
...
- `.agents/docs/OVERVIEW.md`: product scope, major subsystems, boundaries with neighbouring tools (`sccache`, `cargo`, `rustc`), deployment model, and other high-level orientation material.
- `.agents/docs/ARCHITECTURE.md`: repository layout, subsystem boundaries, runtime behaviour, cache-key construction, locking and singleflight model, restore flow, interface contracts with `sccache`, implementation patterns, and operational constraints that matter to engineers.
- Neither document: narrow bug history, one-off migrations, temporary workarounds, or details that are too fine-grained for canonical docs. Project conventions and lint-gate rules belong in `CLAUDE.md`, not in the architecture or overview docs.
...
4. Keep repo-authored documentation style rules: British English, half-width parentheses, and half-width colons.
